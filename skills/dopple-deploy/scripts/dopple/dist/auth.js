 ...
 */
export async function resolveAuth(tokenFlag) {
    // 1. Explicit token flag
    if (tokenFlag) {
        return tokenFlag;
    }
    ...
    if (error || !data.session) {
        throw new Error(`Session refresh failed: ${error?.message || 'no session returned'}. Run "dopple login" again.`);
    }
 ...
 */
export async function whoami(tokenFlag) {
 ...
 */
async function loginHeadless(supabase, authUrl) {
    console.log('');
